...
class RopeSegmenter:
    def __init__(self):
        ...
        self.x_min, self.x_max = -3, 2
        self.y_min, self.y_max = -3, 2
        self.z_min, self.z_max = -1, 2
    ...
    def correct_rope_model(self, rope_points, intersection_1, intersection_2):
        ...
        return translated_rope, translation
    ...
    def read_newest_data(self, file_path):
        ...
        Returns:
            tuple: Parsed data (intersection1, intersection2, tcp) as lists of floats,
                or None if the file is empty, corrupted, or does not exist.
        """
        try:
            ...
            with open(file_path, "r") as file:
                ...
                if lines:
                    # Read the last line and remove any extra spaces/newlines
                    line = lines[-1].strip()
                    
                    # Ensure the line is not empty
                    if not line:
                        print(f"Empty line in file: {file_path}")
                        return None
                    ...
                    try:
                        ...
                        # Validate data length (should be 9 floats)
                        if len(data_list) != 9:
                            print(f"Unexpected data format: {line}")
                            return None
                        
                        # Extract data into segments
                        ...
                        print(f"Data parsing error in file: {file_path}. Error: {ve}")
                        return None
                else:
    ...
    def select_grasp_point(self, corrected_rope_points,translation):
        """
        Allows the user to select a grasp point from the corrected rope points.
        :param corrected_rope_points: numpy array of shape (N, 3) containing 3D coordinates of the rope.
        :return: Selected 3D grasp point.
        """
        # Display the 3D rope points for selection
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(corrected_rope_points[:, 0], corrected_rope_points[:, 1], corrected_rope_points[:, 2],corrected_rope_points[:, 3], c='b', label='Rope Points')
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        plt.title("Select a grasp point")
        plt.legend()
        plt.show(block=False)

        # Allow user to select a point in 3D projection
        print("Please click on the 3D plot to select a grasp point.")
        selected_point = plt.ginput(1)[0]  # Select one point
        plt.close()

        # Add point sender method to another robot arm herer
        # --------------------------------------------------
        #
        # --------------------------------------------------


        # Add translation to the grasp point if translation is set
        if translation is not None:
            selected_point += translation

        print(f"Selected grasp point (with translation): {selected_point}")
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            if frame_names:
                ...
                corrected_rope_points, translation = self.correct_rope_model(interpolated_points,intersection_w_1, intersection_w_2)
                ...
                print('len corrected points', len(corrected_rope_points))
                grasp_robot_2 = corrected_rope_points[int(len(corrected_rope_points)/2)] 
